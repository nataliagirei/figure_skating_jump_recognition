"""
Experiment 5 — R3D-18 frozen backbone, linear probe.

Only classification heads are trained. Failed class excluded.
Best approach found: ~46-48% val accuracy on 3-class problem.
"""
import json
...
REPORT_DIR = ROOT / "reports" / "figures" / "exp5_r3d18_frozen"
...
model = FigureSkatingModelB(num_jump_types=3, dropout_prob=0.3, freeze_backbone=True).to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    for videos, type_labels, rot_labels in train_loader:
        ...
        loss_rot  = criterion_rot(rot_logits[rot_mask], rot_labels[rot_mask] - 1) if rot_mask.any() \
    ...
    save_training_curves(history, REPORT_DIR / "training_curves.png", "R3D-18 Frozen (Linear Probe)")
...
save_history(history, CKPT_DIR / "history_b.json")
print(f"\nR3D-18 frozen training complete. Best val acc: {best_val_acc:.4f}")

# ── Confusion matrix on val set ────────────────────────────────────────────────
model.load_state_dict(torch.load(CKPT_DIR / "best_model_b.pth", map_location=DEVICE, weights_only=True))
model.eval()
y_true, y_pred = [], []
...
save_confusion_matrix(r_true, r_pred, ["single", "double", "triple"],
                      ...
                      "Exp 5: Rotation (1x/2x/3x)", labels=[0, 1, 2])
